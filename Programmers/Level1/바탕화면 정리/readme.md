...
이 문제는 파일을 모두 포함하는 최소 거리의 드래그를 구하는 문제다.
일단 내 생각은 상하좌우 모두 맨 끝 쪽에 있는 파일의 위치를 구하면 되는 문제같다.

기본적으로 wallpaper를 돌면서 파일인 #이 확인될 때마다 상하좌우를 비교해주니 끝났다. 한가지 주의해야하는 점은 가장 아래와 오른쪽 좌표를 구할 때는 배열의 위치보다 1을 더해주어야했고, 때문에 최대값을 비교할 때는 전에 더해준 1을 빼주고 비교해줘야 한다.

***
문제 링크: [링크](https://school.programmers.co.kr/learn/courses/30/lessons/161990)